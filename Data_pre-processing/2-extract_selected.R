...
setwd("/rds/general/user/iw413/home/Summerproject/Scripts")
...
dir.create("../extraction_and_recoding/parameters/codings", showWarnings = FALSE)
...
choices <- read.xlsx("../extraction_and_recoding/parameters/variable_selection_bloods_updated.xlsx")
...
codings <- fread("/rds/general/user/iw413/home/Summerproject/extraction_and_recoding/docs/codings.csv", data.table = FALSE) # Codings for categorical outcomes: https://biobank.ctsu.ox.ac.uk/crystal/exinfo.cgi?src=accessing_data_guide
...
for (i in 1:length(coding_ids)) {
  ...
  write.table(tmp, paste0("../extraction_and_recoding/docs/codings/codes_", codeID, ".txt"),
...
write.table(tmp, paste0("../extraction_and_recoding/docs/codings/codes_template_continuous.txt"),
  row.names = FALSE, col.names = TRUE
)
