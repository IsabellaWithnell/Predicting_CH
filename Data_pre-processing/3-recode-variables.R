...
for (k in 1:ncol(mydata)) {
  ...
  if (!is.na(tmp_coding_id)) {
    ...
    coding <- read.table(paste0("../extraction_and_recoding/docs/codings/codes_", tmp_coding_id, ".txt"), header = TRUE, stringsAsFactors = FALSE)
...
write.xlsx(choices, "../extraction_and_recoding/parameters/parametersnew.xlsx")
